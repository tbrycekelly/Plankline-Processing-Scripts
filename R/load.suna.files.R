...
load.suna.files = function(files, skip = 21, min.size = 1024, verbose = T) {
  ...
  dat = data.frame(Time = conv.time.unix(make.time(year = substr(dat$V2, 1, 4)) + 86400 * as.numeric(substr(dat$V2, 5, 7)) + 3600 * dat$V3),
                   Nitrate = dat$V4)
  message('Done.')
  ...
  for (i in 2:length(files)) {
    ...
    temp = data.frame(Time = conv.time.unix(make.time(year = substr(temp$V2, 1, 4)) + 86400 * as.numeric(substr(temp$V2, 5, 7)) + 3600 * temp$V3),
                     Nitrate = temp$V4)
