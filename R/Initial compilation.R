...
for (i in 1:length(measure.file)) {
  message('Processing file ', i, ' of ', length(measure.file), ' (', round(i*100/length(measure.file)), '%)')
...
for (i in 1:length(class.file)) {
  ...
  if (file.exists(measure.file)) {
    ...
    if (nrow(measure) == nrow(class)) {
      ...
      for (n in unique(class$class)) {
        bin[[n]] = sapply(bin$time, function(x) {sum(class$time == x & class$class == n)})
      }
